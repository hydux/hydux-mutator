...
describe('dynamic updateIn', function () {
    it('simple', function () {
        var type = 'en';
        var newBook = mutator.updateIn(book, function (_) { return _.title[type]; }, function (title) { return title + ' (Original Edition)'; }, { type: type });
        assert.deepEqual(book, originalBook);
        assert.equal(newBook.title.en, 'Harry Potter and the Philosopher\'s Stone (Original Edition)');
        assert(newBook.tags === book.tags);
        assert.deepEqual(newBook.tags, book.tags);
...
function prettyPrint(x) {
    var opts = { depth: null, colors: true };
...
describe('parser', function () {
    ...
    it('lambda', function () {
        ...
        var ast = parse("_ => _.bb.cc['aa'][1][\"dd\"][a1]['cc']");
